...
public class SituatedValidationStocGrad<SAMPLE extends ISituatedDataItem<Sentence, ?>, MR, ESTEP, ERESULT, DI extends ILabeledDataItem<SAMPLE, ?>>
    ...
    protected void parameterUpdate(final DI dataItem,
        ...
        final IJointGraphOutput<MR, ERESULT> parserOutput = graphParser.parse(dataItem.getSample(), dataItemModel);
        stats.mean("model parse", parserOutput.getInferenceTime() / 1000.0,"sec");
        parserOutputLogger.log(parserOutput, dataItemModel, String.format("%d-update", dataItemNumber));
        final List<? extends IJointDerivation<MR, ERESULT>> modelParses = parserOutput.getDerivations();
        final List<? extends IJointDerivation<MR, ERESULT>> bestModelParses = parserOutput.getMaxDerivations();
        ...
        LOG.info("Model parsing time: %.4fsec", parserOutput.getInferenceTime() / 1000.0);
        ...
        if (logConditionedNorm == Double.NEGATIVE_INFINITY) {
            ...
            final IHashVector expectedFeatures = parserOutput.logExpectedFeatures(filter);
        ...
        if (logNorm == Double.NEGATIVE_INFINITY) {
            ...
            final IHashVector expectedFeatures = parserOutput.logExpectedFeatures();
        ...
        if (update.isBad()) {
            LOG.error("Bad update: %s -- log-norm: %f.4f -- feats: %s", update, logNorm, null);
    ...
    public static class Builder<SAMPLE extends ISituatedDataItem<Sentence, ?>, MR, ESTEP, ERESULT, DI extends ILabeledDataItem<SAMPLE, ?>> {
        ...
        public Builder<SAMPLE, MR, ESTEP, ERESULT, DI> setGenlex(ILexiconGenerator<DI, MR, IJointModelImmutable<SAMPLE, MR, ESTEP>> genlex,
                                                                 ICategoryServices<MR> categoryServices) {
            this.genlex = genlex;
        ...
        public Builder<SAMPLE, MR, ESTEP, ERESULT, DI> setLexiconGenerationBeamSize(int lexiconGenerationBeamSize) {
            this.lexiconGenerationBeamSize = lexiconGenerationBeamSize;
            return this;
        }

        public Builder<SAMPLE, MR, ESTEP, ERESULT, DI> setMaxSentenceLength(int maxSentenceLength) {
            this.maxSentenceLength = maxSentenceLength;
            return this;
        }

        public Builder<SAMPLE, MR, ESTEP, ERESULT, DI> setNumIterations(int numIterations) {
        ...
        public Builder<SAMPLE, MR, ESTEP, ERESULT, DI> setTrainingDataDebug(Map<DI, Pair<MR, ERESULT>> trainingDataDebug) {
    ...
    public static class Creator<SAMPLE extends ISituatedDataItem<Sentence, ?>, MR, ESTEP, ERESULT, DI extends ILabeledDataItem<SAMPLE, ?>>
            implements IResourceObjectCreator<SituatedValidationStocGrad<SAMPLE, MR, ESTEP, ERESULT, DI>> {
        ...
        public SituatedValidationStocGrad<SAMPLE, MR, ESTEP, ERESULT, DI> create(
            ...
            final Builder<SAMPLE, MR, ESTEP, ERESULT, DI> builder = new SituatedValidationStocGrad.Builder<SAMPLE, MR, ESTEP, ERESULT, DI>(
                    trainingData,
                    (IJointGraphParser<SAMPLE, MR, ESTEP, ERESULT>) repo.get(ParameterizedExperiment.PARSER_RESOURCE),
                    (IValidator<DI, ERESULT>) repo.get(params.get("validator")));

            if (params.contains("parseLogger")) {
                builder.setParserOutputLogger((IJointOutputLogger<MR, ESTEP, ERESULT>) repo.get(params.get("parseLogger")));
            }

            if (params.contains("genlex")) {
                builder.setGenlex((ILexiconGenerator<DI, MR, IJointModelImmutable<SAMPLE, MR, ESTEP>>) repo.get(params.get("genlex")),
                        (ICategoryServices<MR>) repo.get(ParameterizedExperiment.CATEGORY_SERVICES_RESOURCE));
            }

            if (params.contains("genlexbeam")) {
                builder.setLexiconGenerationBeamSize(Integer.valueOf(params.get("genlexbeam")));
            }

            if (params.contains("maxSentenceLength")) {
                builder.setMaxSentenceLength(Integer.valueOf(params.get("maxSentenceLength")));
            }
